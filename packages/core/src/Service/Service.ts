...
/**
 * 首先Serive继承了EventEmitter，
 * EventEmitter是一个事件总线的库，用于事件订阅触发的操作，
 * 表明service也是一个事件总线管理的库。
 */
// TODO
// 1. duplicated key
export default class Service extends EventEmitter {
  ...
  constructor(opts: IServiceOpts) {
    ...
    // 根据umi提供的获取配置的各种规则获取到用户定义的当前环境下的配置
    ...
    // 通过path2Obj返回后的presets合并对象
    this.initialPresets = resolvePresets({
      ...baseOpts,
      presets: opts.presets || [],
      userConfigPresets: this.userConfig.presets || [],
    });
    // 通过path2Obj返回后的plugins合并对象
